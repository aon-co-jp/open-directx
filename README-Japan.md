...
> **2026-08-08 更新**: 境界チェック付き7項チェーンについて「DXBC側のみ
> あってDXIL側が無い」という非対称が直前のHANDOFFで判明していたため、
> `vector_add_mul_div_sub_add_mul_div_chain7_bounded_dxil.hlsl`を新規に
> `dxc.exe`で実コンパイルし、DXIL版も追加した。NVIDIA GT730実機で
> CPU参照実装との数値一致・境界外要素が書き込まれないことを確認済み
