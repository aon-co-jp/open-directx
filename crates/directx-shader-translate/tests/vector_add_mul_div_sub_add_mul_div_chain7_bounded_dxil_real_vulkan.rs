//! `vector_add_mul_div_sub_add_mul_div_chain7_bounded_dxil.hlsl`(実dxc.exe
//! 出力、DXIL/SM6.0、`if (i < ElementCount) { t1=a[i]+b[i]; t2=t1*a[i];
//! t3=t2/b[i]; t4=t3-a[i]; t5=t4+b[i]; t6=t5*a[i]; c[i]=t6/b[i]; }`という
//! 「境界チェック付き7項演算チェーン」)の実機検証テスト。2026-08-08追加——
//! 直前の2026-08-08 HANDOFFエントリで「境界チェック付き7項はDXBC側のみ、
//! DXIL側は未実装」と明記されていたギャップを埋める。既存の
//! `vector_add_mul_div_sub_add_mul_chain6_bounded_dxil_real_vulkan.rs`
//! (境界チェック付き6項、DXIL)をそのまま7項へ延長した形。
...
const VECTOR_ADD_MUL_DIV_SUB_ADD_MUL_DIV_CHAIN7_BOUNDED_DXIL: &[u8] = include_bytes!(
    "../shaders/vector_add_mul_div_sub_add_mul_div_chain7_bounded_dxil.dxil"
);
...
#[test]
fn dxil_vector_add_mul_div_sub_add_mul_div_chain7_bounded_matches_cpu_reference_and_respects_bounds_on_real_vulkan_hardware(
) {
    let kernel = translate_dxil_chain_to_spirv(VECTOR_ADD_MUL_DIV_SUB_ADD_MUL_DIV_CHAIN7_BOUNDED_DXIL).expect(
        "real dxc-compiled bounded 7-op chain DXIL (cbuffer+icmp ult+conditional br around add-then-mul-then-div-then-sub-then-add-then-mul-then-div) must translate to SPIR-V",
    );
    ...
    device
        ...
        .expect("launch_kernel (DXIL-derived bounded 7-op chain SPIR-V, real Vulkan hardware)");
    ...
    println!(
        "OK: DXIL(dxc.exe実コンパイル、SM6.0、境界チェック付き7項演算チェーン add->mul->div->sub->add->mul->div)->SPIR-V(自前生成、resolve_dxil_calls_and_chainで境界チェックも実解決)->実Vulkan経路が、\
