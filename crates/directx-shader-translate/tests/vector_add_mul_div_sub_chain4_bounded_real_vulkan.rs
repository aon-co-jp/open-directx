//! `vector_add_mul_div_sub_chain4_bounded.hlsl`(実fxc.exe出力、DXBC、
//! `if (i < N) { t1=A[i]+B[i]; t2=t1*A[i]; t3=t2/B[i]; Out[i]=t3-A[i]; }`
//! という「境界チェック付き4項演算チェーン」)の実機検証テスト。2026-08-06
//! 追加——直前のHANDOFFエントリで「境界チェック付き3項のみ検証済み、4項
//! 以上は未検証」と明記されていたギャップを埋める。既存の
//! `vector_add_mul_div_chain3_bounded_real_vulkan.rs`(境界チェック付き
//! 3項)と`vector_sub_div_add_mul_chain4_real_vulkan.rs`(境界チェック無し
//! 4項)の組み合わせに当たる。
...
const VECTOR_ADD_MUL_DIV_SUB_CHAIN4_BOUNDED_DXBC: &[u8] =
    include_bytes!("../shaders/vector_add_mul_div_sub_chain4_bounded.dxbc");
...
#[test]
fn dxbc_vector_add_mul_div_sub_chain4_bounded_matches_cpu_reference_and_respects_bounds_on_real_vulkan_hardware(
) {
    let kernel = translate_chain_shader(VECTOR_ADD_MUL_DIV_SUB_CHAIN4_BOUNDED_DXBC).expect(
        "real fxc-compiled bounded 4-op chain (cbuffer+ult+if+endif around add-then-mul-then-div-then-sub) must translate to SPIR-V",
    );

    assert_eq!(kernel.local_size, (64, 1, 1));
    // N項チェーンの読み込みUAV数はN+1規則(境界チェック無し4項チェーンで5
    // だった実測と同じ規則が境界チェック付きでも成り立つかを確認する)。
    assert_eq!(kernel.read_uav_bind_points.len(), 5);
    ...
    // ゼロ除算を避けるため両方とも正の非ゼロ値に構成(既存のdiv/chain4系
    // 実機テストと同じ方針)。
    ...
    device
        ...
        .expect("launch_kernel (DXBC-derived bounded 4-op chain SPIR-V, real Vulkan hardware)");
    ...
    println!(
        "OK: DXBC(fxc.exe実コンパイル, 境界チェック付き4項演算チェーン add->mul->div->sub)->SPIR-V(自前生成、式木の再帰翻訳+OpSelectionMerge/OpBranchConditional)->実Vulkan経路が、\
