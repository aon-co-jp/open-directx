//! `vector_add_mul_div_chain3_bounded.hlsl`(実fxc.exe出力、DXBC、
//! `if (i < N) { t1 = A[i]+B[i]; t2 = t1*A[i]; Out[i] = t2/B[i]; }`という
//! 「境界チェック付き3項演算チェーン」)の実機検証テスト。2026-08-06追加——
//! 直前のHANDOFFエントリで「境界チェック付きチェーンは2項のみ検証済み、
//! 3項以上は未検証」と明記されていたギャップを埋める。既存の
//! `vector_add_mul_chain_bounded_real_vulkan.rs`(境界チェック付き2項)と
//! `vector_add_mul_div_chain3_real_vulkan.rs`(境界チェック無し3項)の組み
//! 合わせに当たる。
...
const VECTOR_ADD_MUL_DIV_CHAIN3_BOUNDED_DXBC: &[u8] =
    include_bytes!("../shaders/vector_add_mul_div_chain3_bounded.dxbc");
...
#[test]
fn dxbc_vector_add_mul_div_chain3_bounded_matches_cpu_reference_and_respects_bounds_on_real_vulkan_hardware() {
    let kernel = translate_chain_shader(VECTOR_ADD_MUL_DIV_CHAIN3_BOUNDED_DXBC).expect(
        "real fxc-compiled bounded 3-op chain (cbuffer+ult+if+endif around add-then-mul-then-div) must translate to SPIR-V",
    );

    assert_eq!(kernel.local_size, (64, 1, 1));
    // N項チェーンの読み込みUAV数はN+1規則(既存の3項/4項チェーンと同じ、
    // InputA/InputBの再参照分)。境界チェック無し3項チェーンで4だった実測
    // (2026-08-05付エントリ)と同じ規則が境界チェック付きでも成り立つかを
    // 実際に確認する。
    assert_eq!(kernel.read_uav_bind_points.len(), 4);
    ...
    // ゼロ除算を避けるため両方とも正の非ゼロ値に構成(既存の
    // div/chain4系実機テストと同じ方針)。
    ...
    device
        ...
        .expect("launch_kernel (DXBC-derived bounded 3-op chain SPIR-V, real Vulkan hardware)");
    ...
    println!(
        "OK: DXBC(fxc.exe実コンパイル, 境界チェック付き3項演算チェーン add->mul->div)->SPIR-V(自前生成、式木の再帰翻訳+OpSelectionMerge/OpBranchConditional)->実Vulkan経路が、\
