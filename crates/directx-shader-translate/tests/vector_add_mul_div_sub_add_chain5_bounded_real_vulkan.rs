//! `vector_add_mul_div_sub_add_chain5_bounded.hlsl`(実fxc.exe出力、DXBC、
//! `if (i < N) { t1=A[i]+B[i]; t2=t1*A[i]; t3=t2/B[i]; t4=t3-A[i];
//! Out[i]=t4+B[i]; }`という「境界チェック付き5項演算チェーン」)の実機
//! 検証テスト。2026-08-07追加——直前のHANDOFFエントリで「境界チェック
//! 付き4項のみ検証済み、5項以上は未検証」と明記されていたギャップを
//! 埋める。既存の`vector_add_mul_div_sub_chain4_bounded_real_vulkan.rs`
//! (境界チェック付き4項)の組み合わせをそのまま5項へ延長した形。
...
const VECTOR_ADD_MUL_DIV_SUB_ADD_CHAIN5_BOUNDED_DXBC: &[u8] =
    include_bytes!("../shaders/vector_add_mul_div_sub_add_chain5_bounded.dxbc");
...
#[test]
fn dxbc_vector_add_mul_div_sub_add_chain5_bounded_matches_cpu_reference_and_respects_bounds_on_real_vulkan_hardware(
) {
    let kernel = translate_chain_shader(VECTOR_ADD_MUL_DIV_SUB_ADD_CHAIN5_BOUNDED_DXBC).expect(
        "real fxc-compiled bounded 5-op chain (cbuffer+ult+if+endif around add-then-mul-then-div-then-sub-then-add) must translate to SPIR-V",
    );

    assert_eq!(kernel.local_size, (64, 1, 1));
    // N項チェーンの読み込みUAV数はN+1規則(境界チェック付き4項チェーンで5
    // だった実測と同じ規則が5項でも成り立つかを確認する)。
    assert_eq!(kernel.read_uav_bind_points.len(), 6);
    ...
    device
        ...
        .expect("launch_kernel (DXBC-derived bounded 5-op chain SPIR-V, real Vulkan hardware)");
    ...
    println!(
        "OK: DXBC(fxc.exe実コンパイル, 境界チェック付き5項演算チェーン add->mul->div->sub->add)->SPIR-V(自前生成、式木の再帰翻訳+OpSelectionMerge/OpBranchConditional)->実Vulkan経路が、\
         CPU参照実装(((a[i]+b[i])*a[i])/b[i]-a[i]+b[i])と有効範囲{ELEMENT_COUNT}要素すべてで数値一致し、\
