//! FFv1レンジコーダーの128レーン並列化、実機検証(ユーザーのロードマップ
//! 指示: 「64レーン成功後は128レーンに挑戦」)。
//!
//! `range_coder::build_range_decoder_parallel_kernel`はワークグループ
//! 共有メモリ+`OpControlBarrier`のみに依存し、subgroup幅固有の命令を
//! 使わない設計であるため、GT730の`maxComputeWorkGroupInvocations`
//! (`vulkaninfo`で1536を確認済み)の範囲内であれば128レーンでも動く
//! はず、という仮説を実機で検証する。
...
#[test]
fn parallel_range_decoder_scales_to_128_contexts_on_real_vulkan_hardware() {
    let kernel = build_range_decoder_parallel_kernel(CONTEXT_SIZE as u32);
    assert_eq!(kernel.local_size, (128, 1, 1));
    ...
    // 128回分の復号でrefillを何度も踏むよう、十分長いバイト列にする
    // (PADDED_LENの範囲内)。
    let synthetic_bytes: Vec<u8> = (0..200u32).map(|i| ((i * 53 + 7) % 256) as u8).collect();
    let initial_states: [u8; CONTEXT_SIZE] = std::array::from_fn(|i| (10 + (i * 173) % 236) as u8);
    ...
    // 1ワークグループ(128invocation、GT730のsubgroup幅32の4倍)のみ
    // ディスパッチする。
    ...
    device
        ...
        .expect("launch_kernel (128-context parallel range decoder, real Vulkan hardware, chain_n_buffer)");
    ...
    assert_eq!(gpu_bits, cpu_bits.to_vec(), "128-context GPU decoded bits must match CPU reference bit-for-bit");
    ...
    assert_eq!(gpu_final_states, cpu_final_states, "128-context GPU final states must match CPU reference exactly");

    println!(
        "OK: 128レーン並列(GT730のsubgroup幅32の4倍、shared memory+barrier方式)レンジコーダーが実GT730ハードウェア上でCPU参照実装と128コンテキスト分の復号ビット・最終状態の両方で完全一致した"
    );
}
