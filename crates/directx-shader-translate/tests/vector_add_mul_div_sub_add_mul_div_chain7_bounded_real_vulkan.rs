//! `vector_add_mul_div_sub_add_mul_div_chain7_bounded.hlsl`(実fxc.exe出力、
//! DXBC、`if (i < N) { t1=A[i]+B[i]; t2=t1*A[i]; t3=t2/B[i]; t4=t3-A[i];
//! t5=t4+B[i]; t6=t5*A[i]; Out[i]=t6/B[i]; }`という「境界チェック付き7項
//! 演算チェーン」)の実機検証テスト。2026-08-08追加(rs-sync横断セッション)
//! ——直前のHANDOFFエントリで「境界チェック付きチェーンは6項のみ検証済み、
//! 7項以上は未検証」と明記されていたギャップを埋める。既存の
//! `vector_add_mul_div_sub_add_mul_chain6_bounded_real_vulkan.rs`(境界
//! チェック付き6項)の組み合わせをそのまま7項へ延長した形。
...
const VECTOR_ADD_MUL_DIV_SUB_ADD_MUL_DIV_CHAIN7_BOUNDED_DXBC: &[u8] =
    include_bytes!("../shaders/vector_add_mul_div_sub_add_mul_div_chain7_bounded.dxbc");
...
#[test]
fn dxbc_vector_add_mul_div_sub_add_mul_div_chain7_bounded_matches_cpu_reference_and_respects_bounds_on_real_vulkan_hardware(
) {
    let kernel = translate_chain_shader(VECTOR_ADD_MUL_DIV_SUB_ADD_MUL_DIV_CHAIN7_BOUNDED_DXBC).expect(
        "real fxc-compiled bounded 7-op chain (cbuffer+ult+if+endif around add-then-mul-then-div-then-sub-then-add-then-mul-then-div) must translate to SPIR-V",
    );

    assert_eq!(kernel.local_size, (64, 1, 1));
    // N項チェーンの読み込みUAV数はN+1規則(境界チェック付き6項チェーンで7
    // だった実測と同じ規則が7項でも成り立つかを確認する)。
    assert_eq!(kernel.read_uav_bind_points.len(), 8);
    ...
    device
        ...
        .expect("launch_kernel (DXBC-derived bounded 7-op chain SPIR-V, real Vulkan hardware)");
    ...
    println!(
        "OK: DXBC(fxc.exe実コンパイル, 境界チェック付き7項演算チェーン add->mul->div->sub->add->mul->div)->SPIR-V(自前生成、式木の再帰翻訳+OpSelectionMerge/OpBranchConditional)->実Vulkan経路が、\
         CPU参照実装(((((a[i]+b[i])*a[i])/b[i]-a[i]+b[i])*a[i])/b[i])と有効範囲{ELEMENT_COUNT}要素すべてで数値一致し、\
