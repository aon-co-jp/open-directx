//! `vector_add_mul_div_sub_add_mul_div_sub_add_chain9_bounded.hlsl`(実
//! fxc.exe出力、DXBC、境界チェック付き9項演算チェーン
//! add->mul->div->sub->add->mul->div->sub->add)の実機検証テスト。
//! 2026-08-08追加——直前のHANDOFFエントリ「9項以上への拡張は未着手」を
//! 解消。既存の`..._chain8_bounded_real_vulkan.rs`(境界チェック付き
//! 8項)の組み合わせをそのまま9項へ延長した形。
...
const VECTOR_CHAIN9_BOUNDED_DXBC: &[u8] =
    include_bytes!("../shaders/vector_add_mul_div_sub_add_mul_div_sub_add_chain9_bounded.dxbc");
...
#[test]
fn dxbc_vector_add_mul_div_sub_add_mul_div_sub_add_chain9_bounded_matches_cpu_reference_and_respects_bounds_on_real_vulkan_hardware(
) {
    let kernel = translate_chain_shader(VECTOR_CHAIN9_BOUNDED_DXBC).expect(
        "real fxc-compiled bounded 9-op chain must translate to SPIR-V",
    );

    assert_eq!(kernel.local_size, (64, 1, 1));
    assert_eq!(kernel.read_uav_bind_points.len(), 10);
    ...
    device
        ...
        .expect("launch_kernel (DXBC-derived bounded 9-op chain SPIR-V, real Vulkan hardware)");
    ...
    println!(
        "OK: DXBC(fxc.exe実コンパイル, 境界チェック付き9項演算チェーン add->mul->div->sub->add->mul->div->sub->add)->SPIR-V(自前生成、式木の再帰翻訳+OpSelectionMerge/OpBranchConditional)->実Vulkan経路が、\
         CPU参照実装と有効範囲{ELEMENT_COUNT}要素すべてで数値一致し、\
