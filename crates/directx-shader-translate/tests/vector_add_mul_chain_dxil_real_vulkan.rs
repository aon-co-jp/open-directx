//! `vector_add_mul_chain_dxil.hlsl`(実dxc.exe出力、DXIL/SM6.0、
//! `t = A[i]+B[i]; Out[i] = t*A[i];`という2項演算2回のチェーン)の実機検証
//! テスト。既存の`vector_add_dxil_real_vulkan.rs`と同じパターン(実GPU/
//! Vulkanドライバが無ければ`eprintln!`してスキップ、フェイク成功にしない)。
//! DXBC側の`vector_add_mul_chain_real_vulkan.rs`のDXIL版——`translate_shader`
//! ではなく新設した`translate_dxil_chain_to_spirv`(N個の逐次2項演算パターン
//! クラスのDXIL版)を使う点が異なる。
...
#[test]
fn dxil_vector_add_mul_chain_matches_cpu_reference_on_real_vulkan_hardware() {
    let kernel = translate_dxil_chain_to_spirv(VECTOR_ADD_MUL_CHAIN_DXIL)
        .expect("real dxc-compiled DXIL 2-op chain (add then mul) must translate to SPIR-V");
    ...
    // 注: DXBC側チェーンテストと同じ理由で、`opencuda-vulkan::VulkanDevice::
    // launch_kernel`のカーネル名ディスパッチャは"vector_add"名の引数配線
    // (3バッファ+push constant 1個のuint)をそのまま再利用する。実行される
    // 演算はSPIR-Vバイト列側(DXILから生成した実際の`(a+b)*a`チェーン)で決まる。
    ...
    println!(
        "OK: DXIL(dxc.exe実コンパイル、SM6.0、2項演算2回のチェーン)->SPIR-V(自前生成、resolve_dxil_calls_and_chainで式木を実解決)->実Vulkan経路が、CPU参照実装((a[i]+b[i])*a[i])と{N}要素すべてで数値一致した"
    );
