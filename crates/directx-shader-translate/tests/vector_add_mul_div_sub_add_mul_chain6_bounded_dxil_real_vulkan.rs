//! `vector_add_mul_div_sub_add_mul_chain6_bounded_dxil.hlsl`(実dxc.exe
//! 出力、DXIL/SM6.0、`if (i < ElementCount) { t1=a[i]+b[i]; t2=t1*a[i];
//! t3=t2/b[i]; t4=t3-a[i]; t5=t4+b[i]; c[i]=t5*a[i]; }`という「境界チェック
//! 付き6項演算チェーン」)の実機検証テスト。2026-08-07追加——直前の
//! HANDOFFエントリで「境界チェック付きチェーンは5項のみ検証済み、6項
//! 以上は未検証」と明記されていたギャップを埋める。既存の
//! `vector_add_mul_div_sub_add_chain5_bounded_dxil_real_vulkan.rs`(境界
//! チェック付き5項)をそのまま6項へ延長した形。
...
const VECTOR_ADD_MUL_DIV_SUB_ADD_MUL_CHAIN6_BOUNDED_DXIL: &[u8] =
    include_bytes!("../shaders/vector_add_mul_div_sub_add_mul_chain6_bounded.dxil");
...
#[test]
fn dxil_vector_add_mul_div_sub_add_mul_chain6_bounded_matches_cpu_reference_and_respects_bounds_on_real_vulkan_hardware(
) {
    let kernel = translate_dxil_chain_to_spirv(VECTOR_ADD_MUL_DIV_SUB_ADD_MUL_CHAIN6_BOUNDED_DXIL).expect(
        "real dxc-compiled bounded 6-op chain DXIL (cbuffer+icmp ult+conditional br around add-then-mul-then-div-then-sub-then-add-then-mul) must translate to SPIR-V",
    );
    ...
    device
        ...
        .expect("launch_kernel (DXIL-derived bounded 6-op chain SPIR-V, real Vulkan hardware)");
    ...
    println!(
        "OK: DXIL(dxc.exe実コンパイル、SM6.0、境界チェック付き6項演算チェーン add->mul->div->sub->add->mul)->SPIR-V(自前生成、resolve_dxil_calls_and_chainで境界チェックも実解決)->実Vulkan経路が、\
