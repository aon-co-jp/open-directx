//! `vector_add_mul_div_sub_add_mul_div_sub_add_chain9_bounded_dxil.hlsl`
//! (実dxc.exe出力、DXIL/SM6.0、境界チェック付き9項演算チェーン
//! add->mul->div->sub->add->mul->div->sub->add)の実機検証テスト。
//! 2026-08-08追加——直前のHANDOFFエントリ「9項以上への拡張は未着手」を
//! 解消。既存の`..._chain8_bounded_dxil_real_vulkan.rs`(境界チェック付き
//! 8項、DXIL)をそのまま9項へ延長した形。
...
const VECTOR_CHAIN9_BOUNDED_DXIL: &[u8] = include_bytes!(
    "../shaders/vector_add_mul_div_sub_add_mul_div_sub_add_chain9_bounded_dxil.dxil"
);
...
#[test]
fn dxil_vector_add_mul_div_sub_add_mul_div_sub_add_chain9_bounded_matches_cpu_reference_and_respects_bounds_on_real_vulkan_hardware(
) {
    let kernel = translate_dxil_chain_to_spirv(VECTOR_CHAIN9_BOUNDED_DXIL).expect(
        "real dxc-compiled bounded 9-op chain DXIL must translate to SPIR-V",
    );
    ...
    device
        ...
        .expect("launch_kernel (DXIL-derived bounded 9-op chain SPIR-V, real Vulkan hardware)");
    ...
    println!(
        "OK: DXIL(dxc.exe実コンパイル、SM6.0、境界チェック付き9項演算チェーン add->mul->div->sub->add->mul->div->sub->add)->SPIR-V(自前生成、resolve_dxil_calls_and_chainで境界チェックも実解決)->実Vulkan経路が、\
